...
class CartViewModel : ObservableObject {
    ...
    func serviceCallRemove(cObj: CartItemModel){
        ServiceCall.post(parameter: ["cart_id": cObj.cartId, "prod_id": cObj.prodId ], path: Globs.SV_REMOVE_CART, isToken: true ) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: KKey.status) as? String ?? "" == "1" {
                    self.errorMessage = response.value(forKey: KKey.message) as? String ?? "Successfully"
                    self.showError = true
                    self.serviceCallList()
                }else{
                    self.errorMessage = response.value(forKey: KKey.message) as? String ?? "Fail"
    ...
    func serviceCallAddToCart(prodId: Int, qty: Int, didDone: ((_ isDone: Bool,_ message: String  )->())? ) {
        for product in listArr {
            if product.prodId == prodId {
                serviceCallUpdateQty(cObj: product, newQty: product.qty + 1)
                return
            }else{
                break
            }
